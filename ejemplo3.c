...
int main(){
    int *arreglo=NULL, *arreglo2=NULL, num, cont; //asignar null a un apuntador
    ...
    if(arreglo != NULL){        
        ...
        num *=2;   //se crece el arreglo
        arreglo2 = (int*)realloc(arreglo, num*sizeof(int)); //verificar el tamaño de memoria que ya se reeservo
        if(arreglo2 != NULL){ 
            arreglo = arreglo2;       
            for(; cont <num; cont++){   //se agregan los elementos faltantes
                printf("Ingresa el elemento %d del conjunto\n", cont+1);
